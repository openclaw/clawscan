...
### Relyable and the sandbox

Relyable is registered as a built-in adapter but **not preinstalled** in the
`clawscan-runtime` image: the image stays lean, and because grading can execute
skill code, installing the tool is an explicit operator step.

- **Host mode:** `clawscan install relyable`, then
  `clawscan <skill> --scanner relyable --sandbox off`. Without
  `RELYABLE_SCAN_ALLOW_HOST_EXEC=1` relyable stays fail-closed: the `self_spec`
  lane reports `UNJUDGEABLE_NO_SANDBOX` per tool and nothing executes.
- **Docker mode:** build a derived image and select it explicitly:

  ```dockerfile
  FROM ghcr.io/openclaw/clawscan-runtime:latest
  RUN uv tool install "git+https://github.com/veriker/relyable.git@<ref>" --with pytest \
      && ln -sf /root/.local/bin/relyable-scan /usr/local/bin/relyable-scan
  ```

  ```bash
  clawscan <skill> --scanner relyable --sandbox docker --sandbox-image <your-image>
  ```

  (`--with pytest` because relyable re-runs an author's shipped test suite via
  its own interpreter, and uv tool venvs are isolated.) The Docker sandbox is
  the disposable-host acknowledgement, so the adapter passes `--allow-host-exec`
  only in this mode. Against the stock image the run records an explicit
  `failed` scanner result pointing back at this section, and nothing executes.

  A custom profile can carry the image so invocations stay short:
