...
`aig` is different from command-backed scanners. `clawscan install aig` does not
install a local scanner binary, and benchmark CI does not start an A.I.G sidecar.
Start A.I.G separately on localhost or a private network, then run the
[manual smoke path](docs/aig.md#manual-smoke-test).
